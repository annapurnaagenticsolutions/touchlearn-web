...
# Regex to find existing back buttons
# Matches <button ...> ... ← Back ... </button>
regex = re.compile(r'<button\s+[^>]*onClick=\{[^}]*onBack\(\)[^}]*\}[^>]*>[\s\n]*← Back[\s\n]*</button>', re.DOTALL)
...
for filename in os.listdir(directory):
    if filename.endswith(".tsx"):
        ...
        new_content, count = regex.subn(back_button_replacement, content)
        if count > 0:
